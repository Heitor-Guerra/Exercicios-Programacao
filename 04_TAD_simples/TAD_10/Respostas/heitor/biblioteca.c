...
tBiblioteca inicializarBiblioteca() {
  tBiblioteca biblioteca = {0, 0};
  return biblioteca;
}

/**
 * Adiciona um livro à biblioteca.
 * Se estiver dentro do tamanho máximo de livros, imprime "Livro adicionado com
 * sucesso!" Senão, imprime "A biblioteca está cheia. Não é possível adicionar
 * mais livros."
 *
...
tBiblioteca adicionarLivroNaBiblioteca(tBiblioteca biblioteca, tLivros livro) {
  if (biblioteca.tamanho >= MAX_LIVROS) {
    puts("A biblioteca esta cheia. Nao eh possivel adicionar mais livros.");
    return biblioteca;
  }
...
tBiblioteca removerLivroDaBiblioteca(tBiblioteca biblioteca, char *titulo) {
  ...
  puts("Livro nao encontrado na biblioteca.");
  return biblioteca;
}
