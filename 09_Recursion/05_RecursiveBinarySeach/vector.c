...
// Retorna o Ã­ndice do maior elemento do vector (assumindo que podemos comparar
...
// Retorna o Ã­ndice do menor elemento do vector (assumindo que podemos comparar
// elementos usando o operador "<")
...
int binary_seach_rec(Vector *v, int start, int end, data_type val) {
  if (start > end) {
    return -1;
  }
  int mid = (start + end) / 2;
  if (v->data[mid] > val) {
    return binary_seach_rec(v, start, mid - 1, val);
  } else if (v->data[mid] < val) {
    return binary_seach_rec(v, mid + 1, end, val);
  } else {
    return mid;
  }
}
...
int vector_binary_search(Vector *v, data_type val) {
  ...
  return binary_seach_rec(v, start, end, val);
}
