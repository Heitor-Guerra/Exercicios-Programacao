...
float product_get_price_with_discount(void *product) {
    Product *p = (Product*)product;
    return p->price * (1.0-p->discount);
}
void product_print(void *product) {
    Product *p = (Product*)product;
    printf("Product(%s, %.2f, %.2f, %.2f, %d, %d)\n", p->name, p->price, p->discount, product_get_price_with_discount(product), p->qtd, p->sales);
}
