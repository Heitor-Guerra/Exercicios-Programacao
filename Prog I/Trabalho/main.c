...
#define MAX_LARGURA                                                            \
  102 // largura + 2 para laterais      //Ao longo do codigo: y = pista*3 + 1, e
      // largura + 2
...
typedef struct {
  ...
  char personagensSprites[5][2]
                         [4]; // 0-3 carro e animacoes,  4 galinha, cada um com
                              // 2 linhas e 4 colunas (3 chars e \0)
  tGalinha galinha;
...
tGalinha InicializaGalinha(int x, int vidas,
                           int quantidadePistas) { // Funcao Galinha
  tGalinha galinha;
  galinha.x = x;
...
tPista InicializaPista(char direcao, int velocidade, int numCarros,
                       int CarrosX[]) { // Funcao Pista
  int i;
  tPista pista;
  ...
  for (i = 0; i < jogo.quantidadePistas * 3 + 1; i++) {
    for (j = 0; j < jogo.largura + 2; j++) {
      if (j == 0 || j == jogo.largura + 1) { // Desenha as laterais
        jogo.grade[i][j] = '|';
      } else if ((i % 3 == 0 && j % 3) || i == 0 ||
                 i == jogo.quantidadePistas *
                          3) { // Linha e multiplo de 3, e a coluna nao e
                               // //Desenha as divisorias de pista
        jogo.grade[i][j] = '-';
...
tJogo InicializaJogo(char *argv) { // Funcao Jogo
  ...
  fscanf(arquivoConfig, "%d %d %d", &jogo.animacoes, &jogo.largura,
         &jogo.quantidadePistas);
  ...
  for (i = 0; i < jogo.quantidadePistas; i++) {
    ...
    if (pistaDirecao == 'D' || pistaDirecao == 'E') {
      ...
      jogo.pistas[i] = InicializaPista(pistaDirecao, pistaVelocidade,
                                       pistaNumCarros, CarrosX);
    } else if (pistaDirecao == 'G') {
      fscanf(arquivoConfig, "%d", &xGalinha);
      fscanf(arquivoConfig, "%d", &galinhaVidas);
      jogo.galinha =
          InicializaGalinha(xGalinha, galinhaVidas, jogo.quantidadePistas);
  ...
  fscanf(arquivoPersonagens, "%3[^\n]\n",
         jogo.personagensSprites[4][1]); // OBS: Necessario ter %3[^\n]\n para
                                         // quando tiver espacos no sprite
  for (i = 0; i < 4; i++) { // Caso adicione mais sprites de personagem, alterar
                            // aqui e nos desenhos
    for (j = 0; j < 2; j++) {
      fscanf(arquivoPersonagens, "%3[^\n]\n",
...
tGalinha MovimentaGalinha(tGalinha galinha, char mov, int qtdPistas) {
  if (mov == 'w') {
    galinha.y -= 3; // 3 porque a largura da pista e de 3 linhas
             ...
             galinha.y < qtdPistas * 3 - 2) { // Verifica se o movimento e "s",
                                              // e se tem espaco abaixo
    galinha.y += 3;
  }
  return galinha;
}

tGalinha ResetaGalinha(
    tGalinha galinha,
    int qtdPistas) { // Reseta o y e tira uma vida (quando e atropelada)
  galinha.vidas--;
  galinha.y = qtdPistas * 3 - 2;
  return galinha;
}

int ObtemXGalinha(tGalinha galinha) { return galinha.x; }

int ObtemYGalinha(tGalinha galinha) { return galinha.y; }

int ObtemVidasGalinha(tGalinha galinha) { return galinha.vidas; }

void DesenhaGalinha(tGalinha galinha, char mapa[MAX_ALTURA][MAX_LARGURA],
                    char desenhos[5][2][4]) {
  int i;
  for (i = -1; i <= 1;
       i++) { // Comeca em -1 porque a posicao central e a posicao da galinha
  ...
  case 'D':
    for (i = 0; i < pista.numCarros; i++) {
      ...
      if (pista.carrosX[i] > largura) {
        pista.carrosX[i] -=
            largura; // Aparece do outro lado ao ultrapassar a borda
      }
    }
    break;
  case 'E':
...
int VerificaColisaoNaPista(tPista pista, tGalinha galinha, int largura) {
  ...
  for (i = 0; i < pista.numCarros; i++) {
    if (abs(pista.carrosX[i] - xGalinha) <= 2) {
      return i + 1; // Verifica se a distancia entre a galinha e algum carro
                    // causa colisao e retorna o carro(comecando do 1)
    } else if ((xGalinha == 2 && pista.carrosX[i] == largura) ||
               (xGalinha == largura - 1 && pista.carrosX[i] == 1)) {
      return i +
             1; // Verifica se a galinha esta numa extremidade e o carro acabou
                // de atravessar a borda e retorna o carro(comecando do 1)
    }
  }
  return 0;
}

void DesenhaCarrosNaPista(tPista pista, char mapa[MAX_ALTURA][MAX_LARGURA],
                          int numPista, int largura, int animacao, int iteracao,
                          char desenhos[5][2][4]) { //
  int i, j;
  ...
  for (j = 0; j < pista.numCarros; j++) {
    if (pista.carrosX[j] == 1) { // Verifica se o x do carro esta na borda
                                 // esquerda e desenha a ponta do outro lado
      for (i = 0; i <= 1; i++) {
    ...
    } else if (pista.carrosX[j] ==
               largura) { // Verifica se o x do carro esta na borda direita e
                          // desenha a ponta do outro lado
      for (i = -1; i <= 0; i++) {
        mapa[y][pista.carrosX[j] + i] = desenhos[ani][0][i + 1];
        mapa[y + 1][pista.carrosX[j] + i] = desenhos[ani][1][i + 1];
      }
...
tAtropelamentos AtribuiInfo(int idPista, int idCarro, int iteracao, int x,
                            int y) {
...
void OrdenaAtropelamentos(tAtropelamentos atropelamentos[],
                          int numAtropelamentos) { // Ordenacao BubbleSort
  int i, pronto = 0;
  while (!pronto) {
    for (i = 1; i < numAtropelamentos; i++) {
      if (atropelamentos[i - 1].idPista > atropelamentos[i].idPista) {
        ...
        if (atropelamentos[i - 1].idCarro > atropelamentos[i].idCarro) {
          TrocaPosicaoVetor(atropelamentos, i - 1, i);
        } else if ((atropelamentos[i - 1].idCarro ==
                    atropelamentos[i].idCarro) &&
                   atropelamentos[i - 1].iteracao <
...
void GeraInicializacaoTXT(tJogo jogo, char *argv) {
  ...
  for (i = 0; i < jogo.quantidadePistas; i++) {
    DesenhaCarrosNaPista(jogo.pistas[i], jogo.grade, i, jogo.largura,
                         jogo.animacoes, jogo.iteracao,
                         jogo.personagensSprites);
  }

  for (i = 0; i < jogo.quantidadePistas * 3 + 1; i++) {
    for (j = 0; j < jogo.largura + 2; j++) {
      fprintf(arquivoInicializacao, "%c",
              jogo.grade[i][j]); // Mesmo codigo pra printar normal, mas
                                 // trocando por fprintf
    }
    fprintf(arquivoInicializacao, "\n");
  }
  fprintf(arquivoInicializacao,
          "A posicao central da galinha iniciara em (%d %d).",
          ObtemXGalinha(jogo.galinha), ObtemYGalinha(jogo.galinha));
  fclose(arquivoInicializacao);
}

void GeraResumoTXT(tAtropelamentos atropelamentos[], int numAtropelamentos,
                   int iteracaoFinal, char *argv) {
  int i;
  char diretorio[MAX_CAMINHO];
  FILE *arquivoResumo;
  ...
  for (i = 0; i < numAtropelamentos; i++) {
    fprintf(
        ...
        atropelamentos[i].iteracao, atropelamentos[i].idPista,
        atropelamentos[i].idCarro, atropelamentos[i].x, atropelamentos[i].y);
  }
...
void GeraRankingTXT(tAtropelamentos atropelamentos[], int numAtropelamentos,
  ...
  for (i = 0; i < numAtropelamentos; i++) {
    fprintf(arquivoRanking, "%d,%d,%d\n", atropelamentos[i].idPista,
            atropelamentos[i].idCarro, atropelamentos[i].iteracao);
  }
  fclose(arquivoRanking);
}

void GeraEstatisticaTXT(int numMov, int numMovS, int yMax, int yMaxAtr,
                        int yMinAtr, char *argv) {
  char diretorio[MAX_CAMINHO];
...
int AcabouJogo(tJogo jogo) {
  return (ObtemVidasGalinha(jogo.galinha) == 0 ||
          ObtemYGalinha(jogo.galinha) == 1);
}

void DesenhaMapaJogo(
    tJogo jogo, int pontos) { // Desenha e printa o mapa  //Como a funcao e void
                              // nao causa alteracao permanente no grid
  int i, j;
  ...
  printf("Pontos: %d | Vidas: %d | Iteracoes: %d\n", pontos,
         ObtemVidasGalinha(jogo.galinha), jogo.iteracao);
...
void AtualizaHeatmap(tJogo jogo, int linhas, int colunas,
                     int heatmap[linhas][colunas], int colisao) {
  int i, j;
  int x = ObtemXGalinha(jogo.galinha) - 1,
      y = ObtemYGalinha(jogo.galinha) -
          1; // -1 porque comeca no 0, enquanto no jogo comeca em 1
  if (colisao) {
    for (i = 0; i < colunas; i++) {
      heatmap[y][i] = -1; //
      heatmap[y + 1][i] = -1;
    ...
    for (i = 0; i <= 1; i++) {
      for (j = -1; j <= 1; j++) {
        if (heatmap[y + i][x + j] >= 0 && heatmap[y + i][x + j] < 99) {
          heatmap[y + i]
                 [x + j]++; // Incrementa o valor no retangulo 2x3 da galinha
        }
      }
    }
  }
...
void JogaJogo(tJogo jogo, char *argv) {
  ...
  int linhasHeatmap = jogo.quantidadePistas * 3 - 1,
      colunasHeatmap = jogo.largura; // Desconsiderar as bordas
  ...
  AtualizaHeatmap(jogo, linhasHeatmap, colunasHeatmap, heatmap,
                  0); // Contar a posicao Inicial

  int numMov = 0, numMovS = 0, yMaxGalinha = 0, yMaxAtr = 0,
      yMinAtr = 0; // Numeros arbitrarios
  ...
  while (!AcabouJogo(jogo)) {
    ...
    jogo.galinha =
        MovimentaGalinha(jogo.galinha, movimento, jogo.quantidadePistas);
    for (i = 0; i < jogo.quantidadePistas; i++) {
    ...
    /*-------------------------------Atropelamentos-------------------------------*/
    carroAtr = VerificaColisaoNaPista(jogo.pistas[pistaGalinha], jogo.galinha,
                                      jogo.largura);
    if (carroAtr) { // Se nao tiver colisao, retorna 0, do contrario retorna o
                    // numero do carro
      atropelamentos[numAtr] = AtribuiInfo(
          pistaGalinha + 1, carroAtr, jogo.iteracao + 1,
          ObtemXGalinha(jogo.galinha),
          ObtemYGalinha(
              jogo.galinha)); // Pista + 1, pq pistaGalinha comeca em 0;
                              // Iteracao + 1, pq a iteracao adiciona no final
      numAtr++;
      AtualizaHeatmap(jogo, linhasHeatmap, colunasHeatmap, heatmap, 1);

      if ((jogo.quantidadePistas * 3 - ObtemYGalinha(jogo.galinha)) >
          yMaxAtr) { // Diferenca da altura do mapa(ponto 1 ate borda inferior)
                     // e da galinha
        yMaxAtr = jogo.quantidadePistas * 3 - ObtemYGalinha(jogo.galinha);
      }
      if ((jogo.quantidadePistas * 3 - ObtemYGalinha(jogo.galinha)) < yMinAtr ||
          yMinAtr == 0) { // Diferenca da altura do mapa(ponto 1 ate borda
                          // inferior) e da galinha
      ...
      pontos = 0;
      if (jogo.animacoes) {
        jogo.pistas[pistaGalinha] =
            ReduzVelocidadePista(jogo.pistas[pistaGalinha]);
      }
     ...
     * Heatmap---------------------------*/
    if ((jogo.quantidadePistas * 3 - ObtemYGalinha(jogo.galinha)) >
        yMaxGalinha) { // Diferenca da altura do mapa(ponto 1 ate borda
                       // inferior) e da galinha
