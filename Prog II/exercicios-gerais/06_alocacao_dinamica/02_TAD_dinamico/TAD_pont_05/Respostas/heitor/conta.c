 ...
 */
tConta *CriaConta() {
  tConta *cont = (tConta *)malloc(sizeof(tConta));
  cont->user = CriaUsuario();
  return cont;
}
 ...
 */
int VerificaConta(tConta *conta, int numero) { return conta->numero == numero; }
 ...
 */
void DepositoConta(tConta *conta, float valor) { conta->saldo += valor; }
