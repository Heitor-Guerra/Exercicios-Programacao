 ...
 *
 * @return Ponteiro para o filme criado ou encerra caso não seja possível alocar
 * memória.
 */
tFilme *CriarFilme() {
...
void LeFilme(tFilme *filme, int codigo) {
  filme->cod = codigo;
  scanf("%[^,],%d,%d\n", filme->nome, &filme->preco, &filme->qtdEstoque);
}
 ...
 */
void DestruirFilme(tFilme *filme) { free(filme); }
 ...
 */
int ObterCodigoFilme(tFilme *filme) { return filme->cod; }
 ...
 */
int ObterValorFilme(tFilme *filme) { return filme->preco; }
 ...
 * -1 se o nome do primeiro filme for menor que o nome do segundo filme, 0 se os
 * nomes forem iguais.
 */
int CompararNomesFilmes(tFilme *filme1, tFilme *filme2) {
  int res = strcmp(filme1->nome, filme2->nome);
  if (res == 0) {
    return 0;
  } else if (res < 0) {
    return -1;
